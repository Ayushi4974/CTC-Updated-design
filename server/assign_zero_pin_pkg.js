...
async function run() {
  console.log('Assigning $100 Zero Pin Package to user CTC43214...');
  ...
  let pkg = await Package.findOne({ isZeroPin: true });
  if (!pkg) {
    pkg = await Package.findOne({ name: 'Package' });
  }

  if (!pkg) {
    console.error('No Zero Pin Package found in the database! Creating one...');
  ...
  console.log(`Using Zero Pin Package: ${pkg.name} (ID: ${pkg._id}), dailyProfit: ${pkg.dailyProfit}%`);
  ...
  console.log(`Created active Zero Pin UserPackage document: ${userPackage._id}`);

  // Update User
  user.activePackage = pkg._id;
  user.totalInvestment = 100;
  user.pins = 0; // Set user pins to 0 as they now have a zero pin package
  user.isActive = true;
  await user.save();
  console.log(`Updated User document (totalInvestment: $100, pins: 0, isActive: true, activePackage set).`);
  ...
  await AuditLog.create({
    ...
    details: {
      ...
      reason: 'Assigned Zero Pin package via script'
    }
  });
  ...
  await Transaction.create({
    ...
    type: 'deposit',
    amount: 100,
    txHash: 'MANUAL_SCRIPT_ASSIGN_ZEROPIN',
    status: 'success',
    description: `Manual Zero Pin package assignment: ${pkg.name}`
  });
  console.log('Created Transaction log entry.');

  console.log('Zero Pin Package assignment complete successfully!');
  process.exit(0);
}
